...
JavaScript начинался с небольшой роли в сети, в основном HTML. Сегодня он огромен, и некоторые веб-сайты почти полностью созданы с
помощью JavaScript. Чтобы сделать JavaScript более модульным, чистым и удобным в сопровождении; ES6 представил способ легкого обмена 
кодом между файлами JavaScript. Это включает в себя экспорт частей файла для использования в одном или нескольких других файлах и импорт 
нужных частей там, где они вам нужны. Чтобы воспользоваться этой функцией, вам необходимо создать скрипт в HTML-документе с typeрасширением 
module. Вот пример:
<script type="module" src="filename.js"></script>
Сценарий, использующий этот moduleтип, теперь может использовать функции importи export, о которых вы узнаете в предстоящих испытаниях.
Добавьте сценарий в HTML-документ типа moduleи передайте ему исходный файлindex.js*/
...
Представьте файл с именем math_functions.js, который содержит несколько функций, связанных с математическими операциями.
Один из них хранится в переменной , addкоторая принимает два числа и возвращает их сумму. Вы хотите использовать 
эту функцию в нескольких разных файлах JavaScript. Чтобы поделиться им с этими другими файлами, вам сначала нужно exportэто сделать.
...
export { add };
Когда вы экспортируете переменную или функцию, вы можете импортировать ее в другой файл и использовать без необходимости 
переписывать код. Вы можете экспортировать несколько вещей, повторив первый пример для каждой вещи, которую вы хотите экспортировать, 
или поместив их все в инструкцию экспорта второго примера, например так:
...
importпозволяет выбрать, какие части файла или модуля загружать. В предыдущем уроке примеры экспортировались
addиз math_functions.jsфайла. Вот как вы можете импортировать его для использования в другом файле:

import { add } from './math_functions.js';
Здесь вы importнайдете , импортируете только эту функцию для использования и проигнорируете остальные. 
Сообщает импорту искать файл в той же папке, что и текущий файл. Относительный путь к файлу ( ) и расширение файла ( ) 

необходимы при использовании импорта таким образом.addmath_functions.js./math_functions.js./.js

Вы можете импортировать более одного элемента из файла, добавив их в importоператор следующим образом:

import { add, subtract } from './math_functions.js';
Добавьте соответствующий importоператор, который позволит текущему файлу использовать функции uppercaseStringи 
lowercaseString, которые вы экспортировали в предыдущем уроке. Эти функции находятся в файле с именем string_functions.js,
который находится в том же каталоге, что и текущий файл.*/
...
Предположим, у вас есть файл, и вы хотите импортировать все его содержимое в текущий файл. Это можно сделать с помощью import * asсинтаксиса. Вот пример, 
когда содержимое указанного файла math_functions.jsимпортируется в файл в том же каталоге:

import * as myMathModule from "./math_functions.js";
Приведенный выше importоператор создаст объект с именем myMathModule. Это просто имя переменной, вы 
можете назвать его как угодно. Объект будет содержать все экспорты из math_functions.jsнего, поэтому 
вы можете получить доступ к функциям, как и к любому другому свойству объекта. Вот как можно использовать импортированные функции addи :subtract

myMathModule.add(2,3);
myMathModule.subtract(5,3);
Для кода в этом файле требуется содержимое файла: string_functions.js, который находится в том же каталоге, что и текущий файл.
Используйте import * asсинтаксис для импорта всего из файла в объект с именем stringFunctions.*/
...
На exportуроке вы узнали о синтаксисе, называемом именованным экспортом . Это позволило сделать несколько функций и переменных
доступными для использования в других файлах.

Есть еще один exportсинтаксис, который вам нужно знать, известный как экспорт по умолчанию . Обычно вы будете использовать этот
синтаксис, если из файла экспортируется только одно значение. Он также используется для создания резервного значения для файла или модуля.
...
Поскольку export defaultиспользуется для объявления резервного значения для модуля или файла, вы можете иметь только одно значение, 
которое будет экспортом по умолчанию в каждом модуле или файле. Кроме того, вы не можете использовать export defaultс var, letилиconst
...
В последнем испытании вы узнали о export defaultего использовании. Чтобы импортировать экспорт по умолчанию, вам нужно 
использовать другой importсинтаксис. В следующем примере addэто экспорт файла по умолчанию math_functions.js. Вот как его импортировать:

import add from "./math_functions.js";
Синтаксис отличается одним ключевым местом. Импортированное значение addне заключено в фигурные скобки ( {}). addздесь 
просто имя переменной для любого экспорта файла по умолчанию math_functions.js. Вы можете использовать здесь любое имя 
при импорте значения по умолчанию.

В следующем коде импортируйте экспорт по умолчанию из файла math_functions.js, который находится в том же каталоге, что и этот
файл. Дайте импорту имя subtract.
*/
...
Обещание в JavaScript звучит именно так: вы используете его, чтобы дать обещание что-то сделать, обычно асинхронно.
Когда задача завершается, вы либо выполняете свое обещание, либо не выполняете его. является функцией-конструктором,
поэтому для ее создания Promiseнеобходимо использовать ключевое слово. newВ качестве аргумента принимает функцию с двумя параметрами - resolveи reject.
Это методы, используемые для определения результата обещания. Синтаксис выглядит следующим образом:

const myPromise = new Promise((resolve, reject) => {

});
Создайте новое обещание с именем makeServerRequest. Передайте функцию с параметрами resolveи rejectв конструктор.*/
...
Промис имеет три состояния: pending, fulfilled, и rejected. Обещание, которое вы создали в последнем испытании, навсегда застряло в состоянии, 
pendingпотому что вы не добавили способ выполнить обещание. Для этого используются параметры и, указанные в аргументе обещания resolve.
используется, когда вы хотите, чтобы ваше обещание было успешным, и используется, когда вы хотите, чтобы оно потерпело неудачу. 
Это методы, которые принимают аргумент, как показано ниже.rejectresolvereject
...
В приведенном выше примере в качестве аргумента этих функций используются строки, но на самом деле это может быть что угодно. 
Часто это может быть объект, данные которого вы будете использовать для размещения на своем веб-сайте или в другом месте.

Сделайте так, чтобы обещание учитывало успех и неудачу. Если responseFromServerесть true, вызовите resolveметод для успешного выполнения обещания. 
Передайте resolveстроку со значением We got the data. Если responseFromServerесть false, используйте rejectвместо этого метод и передайте ему строку:
Data not received.
...
Промисы наиболее полезны, когда у вас есть процесс, который занимает неизвестное количество времени в вашем коде 
(например, что-то асинхронное), часто запрос к серверу. Когда вы делаете запрос к серверу, это занимает некоторое время, 
и после его завершения вы обычно хотите что-то сделать с ответом от сервера. Этого можно добиться, используя thenметод. 
Метод thenвыполняется сразу после того, как ваше обещание выполнено с помощью resolve. Вот пример:

myPromise.then(result => {
  
});
resultисходит из аргумента, данного методу resolve.

Добавьте thenметод к вашему обещанию. Используйте resultв качестве параметра его функцию обратного вызова и войдите resultв консоль.*/
...
catchэто метод, используемый, когда ваше обещание было отклонено. Он выполняется сразу после rejectвызова метода обещания. Вот синтаксис:

myPromise.catch(error => {
  
});
errorаргумент, передаваемый методу reject.

Добавьте catchметод к вашему обещанию. Используйте errorв качестве параметра его функцию обратного вызова и войдите errorв консоль.*/
