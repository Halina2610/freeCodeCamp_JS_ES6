...
Вы можете использовать /row row row/, но что, если вы не знаете повторяющееся конкретное слово? Группы захвата можно 
использовать для поиска повторяющихся подстрок.

Группы захвата создаются путем заключения шаблона регулярного выражения в круглые скобки. В этом случае цель состоит
в том, чтобы захватить слово, 
состоящее из буквенно-цифровых символов, поэтому группа захвата будет \w+заключена в круглые скобки: /(\w+)/.

Подстрока, соответствующая группе, сохраняется во временной «переменной», к которой можно получить доступ в том же 
регулярном выражении, используя обратную косую черту и номер группы захвата (например, \1). Группы захвата автоматически
нумеруются по положению открывающих скобок (слева направо), начиная с 1.
...
Используйте группы захвата для reRegexсопоставления строки, состоящей только из одного и того же числа, повторяющегося ровно 
три раза, разделенного одиночными пробелами.*/
...
Вы можете искать и заменять текст в строке, используя .replace()строку. Входные данные для .replace()- это сначала шаблон 
регулярного выражения, который вы хотите найти. Второй параметр — это строка для замены совпадения или функция, которая что-то делает.
...
Вызов replaceвернет строку Camp Code.

Напишите регулярное выражение fixRegex, используя три группы захвата, которые будут искать каждое слово в строке one two three. 
Затем обновите replaceTextпеременную, заменив one two threeее строкой three two one, и присвойте результат переменной result. 
...
Иногда пробельные символы вокруг строк нежелательны, но они есть. Типичная обработка строк заключается в удалении 
